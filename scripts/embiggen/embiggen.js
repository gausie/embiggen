...
var ALWAYS_BLOCKED_SKILLS = $skills(_templateObject1$2 || (_templateObject1$2 = _taggedTemplateLiteral(["Drench Yourself in Sweat, Spirit of Peppermint, Spirit of Cayenne, Spirit of Garlic, Spirit of Wormwood, Spirit of Bacon Grease"])));

// Items mafia can't acquire cleanly, or that we simply never want to use.
var UNWANTED_ITEMS = $items(_templateObject10$2 || (_templateObject10$2 = _taggedTemplateLiteral(["M-242, snake, sparkler, Mer-kin strongjuice, Mer-kin smartjuice, Mer-kin cooljuice, pirate tract, pirate pamphlet, pirate brochure, elven suicide capsule, Ghost Dog Chow, Yummy Tummy bean"])));

/** Skills whose buffs clash with our own class kit, so gaining them would bounce. */
function classConflictSkills() {
  if (kolmafia.myClass() === $class(_templateObject11$2 || (_templateObject11$2 = _taggedTemplateLiteral(["Turtle Tamer"])))) return _toConsumableArray(BLESSINGS);
  if (kolmafia.myClass() === $class(_templateObject12$1 || (_templateObject12$1 = _taggedTemplateLiteral(["Pastamancer"])))) {
    return kolmafia.Thrall.all().map(thrall => thrall.skill).filter(skill => skill !== $skill(_templateObject13$1 || (_templateObject13$1 = _taggedTemplateLiteral(["none"]))));
  }
  return [];
}
...
function lockedHeartstoneSkills() {
  return HEARTSTONE_SKILLS.filter(_ref => {
    var _ref2 = _slicedToArray(_ref, 2),
      skill = _ref2[0],
      pref = _ref2[1];
    return skill !== $skill(_templateObject14 || (_templateObject14 = _taggedTemplateLiteral(["none"]))) && !get(pref, false);
  }).map(_ref3 => {
...
function outOfSeasonItems() {
  ...
  return month < 9 || month > 11 ? [$item(_templateObject15 || (_templateObject15 = _taggedTemplateLiteral(["crystallized pumpkin spice"])))] : [];
}
...
function songSlotsFull() {
  var limit = have($skill(_templateObject16 || (_templateObject16 = _taggedTemplateLiteral(["Mariachi Memory"])))) ? 4 : 3;
